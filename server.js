...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('recalibrate', function (start, end, password) {
        if (password == pass) {
            updateDisplaySize(start, end, robot.getScreenSize().width, robot.getScreenSize().height)
        }
    })

    socket.on('movepointer', function (cpoint, password) {
        if (password == pass) {
            updatePointer(cpoint)
        }
    })

    socket.on('mousebuttontoggle', function (mousestatus, password) {
        if (password == pass) {
            if (!mousestatus) {
